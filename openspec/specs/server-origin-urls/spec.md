# server-origin-urls Specification

## Purpose
Defines how the server derives and uses bind-host, display, and public origin values for
generated endpoint URLs and OAuth metadata, so startup diagnostics and externally advertised
URLs reflect the operator's configured host and origin rather than an internal bind address.
