...
## Purpose
Keeps SQLite schema migrations recoverable and observable, especially destructive migrations
that rebuild large virtual tables such as `documents_vec`. The system runs migrations with
rollback-safe settings by default, reports visible per-step progress, validates destructive
migrations against failure scenarios, and documents backup guidance for high-value databases.
