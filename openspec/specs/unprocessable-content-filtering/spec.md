# unprocessable-content-filtering Specification

## Purpose
Keeps the crawler from spending requests, storage, and failure budget on content no configured pipeline can read, by deriving a single processability predicate from the pipelines themselves and applying it both before a link is queued and once response headers arrive.
