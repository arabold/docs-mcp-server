...
- Sanitize/truncate error messages from binary file parsers
- Never log raw error objects that may contain binary file content
...
- Node 22 environment: Use `test/setup-env.ts` for polyfills (e.g. `localStorage`) if tests crash mysteriously.
...
- Treat all linter/typecheck warnings as errors
- Use `npm run lint:fix` to automatically fix linter issues (add `-- --unsafe` for unsafe fixes like unused imports)
- Run `npm test` to ensure full test suite passes
- Never push changes that fail local verification (avoids CI failures)
...
### Automated Files (DO NOT EDIT)

- `CHANGELOG.md`: Do NOT edit manually. It is auto-generated by CI based on conventional commits.
- `package.json` version: Do NOT increment manually. It is managed by the release process.
